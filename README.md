...
- **论坛侧展示绑定** — 个人资料页显示已绑定的 MC 账号，每篇帖子/回复的作者名旁挂一个 MC 徽章
  （**仅登录用户可见**，游客拿到的资源里根本不带这些字段）。
- **服务器状态页** — 论坛侧 `/mc-bridge/status`：各服在线人数、TPS/MSPT、版本、MOTD、最后心跳与
  最近事件；论坛侧边栏有入口。与绑定页一样是服务端渲染，不需要前端构建。
