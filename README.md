...
- **论坛投票联动** — 直接对接论坛的 **fof/polls** 扩展：管理员在论坛 `/polls` 页面创建投票，
  游戏内自动广播公告，玩家用 `/vote <编号>` 参与，票记到**已绑定的论坛账号**名下，
  论坛页面上能看到同一份投票结果；投票结束后广播最终票数与获胜选项。
  单选/多选、最大可选数量、能否改票等规则完全沿用 fof/polls，插件不重新实现。
