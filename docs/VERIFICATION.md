...
### 2.21 举报讨论的标题 / 标签 / 发布账号可配置 + PlaceholderAPI（0.0.16）

**用户在 2.20 上线后追加的需求**：这三样要能自定义、最好接 PlaceholderAPI；
随后又追加「标签要能多选」，于是标签从单值改成列表。

| 项目 | 内容 |
|------|------|
| 技术约束 | PlaceholderAPI 是 Bukkit/Paper 的 API，**论坛端（PHP）拿不到**。所以标题必须在游戏内渲染完再随举报发给论坛，协议新增可选 `title` 字段 |
| 配置位置 | 三样都放进插件 `config.yml` 的 `report:` 段：`title-format` / `tags` / `actor`，随请求发给论坛；论坛侧设置兜底。**插件配置非空时优先** |
| 标题占位符 | 两类：插件自己的 `{target} {reporter} {reason} {server}`，以及 PlaceholderAPI 的 `%...%`（装了才展开，否则原样保留） |
| 多标签 | `tags` 收逗号分隔列表，每项是 slug 或标签 ID；协议里发**数组**，论坛逐个解析、能解析到的都挂上 |
| 平台边界 | PAPI 的引用被隔离在 `paper/PlaceholderApiHook`（全 jar 唯一提到 PAPI 的类）。没装 PAPI 的服务器永远不会加载它，因此不会 `NoClassDefFoundError`；Velocity 完全不涉及。`verifyJar` 的「共享核心不得引用平台类」断言仍然通过 |
| 依赖 | `paperCompileOnly me.clip:placeholderapi`（新增 extendedclip 仓库）。compileOnly：不进 jar、运行期不要求 |
| 线程 | 标题里的 `%...%` 在**主线程**以举报人身份展开（PAPI 不保证线程安全）；`{tokens}` 在异步线程随举报一起填充 |

**协议兼容**：三个字段全部可选。旧论坛收到新插件的字段会忽略（控制器只读它认识的键）；
新论坛遇到旧插件（不带这些字段）则用它自己的设置。反向亦然。

**两处必须一起改的地方**

1. **写回设置**：解析出的账号与标签会写回论坛设置。这不只是让 `--show` 好看 ——
   2.20 的公告守护正是靠它识别举报讨论，若只存在于单次请求里，举报会被广播进游戏。
2. **多标签下的守护**：`QueueAnnouncement` 现在按 id **列表**匹配（`whereIn`），任一命中即跳过。

**多标签的现实约束**：`flarum/tags` 限制一条讨论能挂的主/次标签数量。超出时创建会抛异常，
被兜底捕获并写进日志 —— 这是**已知且期望**的行为：宁可举报已入库但讨论没建成（并留下日志），
也不该让玩家的 `/report` 变成 500。

**契约同步**：mock 支持 `title` / `tags` / `actor` 三个提示并模拟解析（slug→id、用户名→id）；
协议测试新增 2 组（提示被尊重、无效提示不影响举报），32 → **34 项**。

规模：`verify.mjs` **321 项**、协议 **34 项**、构建自测 **45 项**（37 → 45，新增配置解析断言），
全部通过。

### 2.22 举报超时但实际已提交：幂等重试（0.0.17）

**用户在 2.21 上线后反馈**：游戏内偶尔提示 `无法连接论坛：Cannot reach the forum: Request timed out`，
**但论坛里帖子正常发出来了**。

| 项目 | 内容 |
|------|------|
| 现象 | `/report` 报连接超时，论坛侧却已建成讨论 |
| 含义 | 客户端超时，服务端做完了 —— 客户端拿到的结论是**错的** |
| 真实危害 | 不只是文案难看：玩家以为失败会**再举报一次**，于是同一条举报变成两条记录 + 两条讨论 |
| 为什么现在才出现 | 2.21 之后 `/report` 不再只是「插一行」，它还要同步创建讨论（首帖、标签关联、事件、Markdown 解析）。客户端预算只有 `forum.request-timeout-seconds`（默认 **10 秒**），而这段工作一旦叠加基础设施抖动（PHP-FPM 排队、CDN）就会超 |
| 客户端修法 | 见下：只在「结果未知」时重试，并带上幂等 id |
| 另一处可疑点 | 插件用 Java `HttpClient`，默认经 ALPN 协商 **HTTP/2**。连接在请求飞行途中被代理/CDN 关闭时，请求会一直挂到超时 —— 表现正是「间歇、且服务端其实处理了」。已固定为 **HTTP/1.1**：本桥接只有零星小请求，多路复用没有收益 |

**修法的关键：区分「结果未知」与「结果确定」**

`BridgeException.statusCode()` 恰好区分了这两者：`-1` 表示传输层失败（连不上/超时/响应不可解析），
`>= 0` 表示服务端已答复。于是：

1. 服务端答复（哪怕 500）→ 结果确定，**不重试**（重试只会重复同一个错误）。
2. 完全没有响应 → 结果未知，用**同一个 `report_uid`** 重试一次。
3. 两次都没响应 → 文案改成「&e论坛没有及时回应，举报&f可能已经提交&e。&7为避免重复，请先不要再次举报」——
   一句真话，而不是一个会诱导重复举报的「失败」。

**论坛侧幂等**：`report_uid` 在 Flarum 缓存里认领 600 秒（与签名 nonce 窗口一致，
也就是重试可能到达的时间窗）：

- 认领发生在**入库之后、建讨论之前**，这样重试若恰好在「正在建讨论」时到达，会看到认领记录并直接返回，
  **不会**再插一条；否则两次请求会各建一条讨论。
- 重试返回 `{"ok":true,"report_id":…,"discussion_id":…,"duplicate":true}` —— `ok` 即成功，
  `duplicate` 只是给调用方的额外信息。
- 缓存而非新列：不引入迁移，也不必给 `mc_reports` 加索引；代价是缓存被清空时极端情况下可能重复入库，
  这比每次都加一列要划算。
...
### 2.26 举报讨论「没有上下文」的排查与修复（0.0.21）

**现象**（真实论坛 `d/81`，举报 #14，服务器 `Lobby`）：讨论正文只有举报详情、举报原因和落款，
**完全没有聊天记录**，玩家侧看不出原因。

**排查过程**（每一步都用可验证的证据，而不是猜）：

| 步骤 | 手段 | 结论 |
|------|------|------|
| 1 | 拉 `GET /api/discussions/81` 与 `GET /api/posts?filter[discussion]=81` | 正文里确实连「该玩家的近期公开聊天」这一节都没有；举报详情、原因、落款都正常，说明讨论创建与渲染链路本身是通的 |
| 2 | 举报时间 2026-09-26T12:41:47Z，v0.0.20 发布于 11:51:40Z | 举报发生在发版约 50 分钟后，与「刚更新完在测新功能」吻合 |
| 3 | 探测**只有 0.0.20 才注册**的路由：`GET /api/mc-bridge/reports` 返回 **401**（未签名的机器接口）而不是 404 | **论坛扩展确认已是 0.0.20**（旧版会 404），排除「扩展没更新」 |
| 4 | 插入成功（讨论已建；`context` 列若不存在会 SQL 报错让 `/report` 返回 500） | 迁移已执行，`mc_reports.context` 列存在 |
| 5 | 复核 `Yaml.getInt` 对缺失键的行为 | 正确返回默认值 → 即使服务端仍是**旧版 config.yml**（`saveResource` 从不覆盖已有文件），`chat-context-lines` 也会取到 10，**排除**这一条 |

结论：扩展是新的、列存在、默认值正确 —— 也就是说**游戏侧根本没有把上下文发上来**。
剩下两种可能，都动手处理了：

**修法一（真 bug）：被取消的聊天事件被跳过。** 原实现是
`@EventHandler(priority = MONITOR, ignoreCancelled = true)`。很多聊天格式插件会**取消**
`AsyncChatEvent` 再自己广播一份 —— 在这种服上原实现**一条都记不到**，而表现与「玩家没说过话」
完全一样。改为 **LOWEST 优先级且不跳过取消**：此时事件还没被别的插件取消，`message()` 也还是
玩家自己输入的原文（对记录来说正是想要的）。

**修法二（可诊断性）：把「三种情况长得一模一样」这件事本身修掉。**

| 改动 | 作用 |
|------|------|
| 启动日志加版本号：`McBridge 已启用（平台 {platform}，版本 {version}）…` | 服务端实际加载的是哪个 jar，从此一眼可见（这次为它绕了一圈） |
| 每次举报输出一行：`已附带 N 条聊天上下文`，或未附带的原因（开关为 0 / 开关开着但没捕捉到） | 服务端日志直接回答「这一次带了什么」 |
| `/mcbridge stats` 增加「聊天缓冲：X 名玩家 / Y 条」 | 不必等有人举报，随时能确认采集是否在工作 |
| 举报请求新增 `context_enabled`；扩展据此在正文写「未附带聊天记录：开关是开着的……」 | 讨论正文自己说清是**开关关着**还是**玩家没说话**；旧插件不发该字段时保持旧行为，不回归 |

`context_enabled` 是**创建期提示**、不落库 —— 复用 `ReportDiscussion::create()` 已有的
`$overrides` 机制（与 title/tags/actor 同一路），因此**不需要新迁移**。

**验证到的**：

| 项目 | 证据 |
|------|------|
| 旧版 config.yml 的默认值 | 新增运行时自测：把随包 config.yml 里本次新增的四个键**逐行删掉**（模拟升级后仍是旧文件的服务器），断言 `chat-context-lines` = 10、展示通道 = `[CHAT]`、标题 5 秒、血条 10 秒，且配置仍可用。并先断言「键确实被删掉了」，避免测试自己失效 |
| 诊断输出不会漏占位符 | 既有的 `log.enabled leaves no placeholder behind` 断言在这次改动中**真的失败了**（新增 `{version}` 未传入），同步更新后通过 —— 这条护栏起作用了 |
| 语言键不能藏在表达式里 | `verify.mjs` 第一次跑就报 `log.report-context-empty` / `log.report-context-off`「从未被引用」：键名藏在三元表达式里，检查只认字面量实参。改成 if/else 各写一次后才通过 |
| `context_enabled` 不破坏请求契约 | 协议测试新增：`true` / `false` / 缺省（旧插件）/ 字符串 `"1"` 四种取值下，举报都必须 201 且入库 |
| 构建自测 | **77 项**（68 → 77） |
| 协议测试 | **46 项**（45 → 46） |
| 静态检查 | **384 项**，0 告警 |

**仍未验证**：本地无法启动 Minecraft 服务端，所以「被取消的聊天事件现在能被记录」只有编译与
单元测试级别的保证 —— 需要在一台装了聊天格式插件的服上实测一次（第 4 节清单已加入）。
NeoForge 侧 `ServerChatEvent` 是否在 1.21.1 的签名聊天管线上如实触发，同样只能实机确认。
...
# ---- 聊天上下文（0.0.21）----
# 1) 先确认服务端加载的版本与采集是否工作：
#      日志应出现「McBridge 已启用（平台 paper，版本 0.0.21）…」
#      让被举报人现在公开频道说几句，然后 /mcbridge stats
#      -> 「聊天缓冲」应从 0 变成「1 名玩家 / N 条」；仍是 0 就说明采集没工作
# 2) 若该服装了会取消 AsyncChatEvent 的聊天格式插件，这一项**尤其**要测（0.0.21 修的正是它）
# 3) 举报该玩家：
#      - 讨论正文应出现「该玩家的近期公开聊天」+ 代码块
#      - 服务端日志应输出「举报 <玩家> 已附带 N 条聊天上下文」
# 4) 若正文写「未附带聊天记录：游戏服务器上的开关是开着的…」
#      = 开关开着，但该玩家在被举报前确实没在公开频道说过话
# 5) 若正文里完全没有这一节
#      = 开关是 0，或服务端插件仍是旧版 —— 看启动日志里的版本号即可确认
# 6) 对照检查：私聊（/msg）与命令（/...）**不应**出现在记录里
