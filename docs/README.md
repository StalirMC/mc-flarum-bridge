...
管理员可以在论坛发起一个活动投票（例如"下次活动玩什么？"，带 2–10 个选项），
发起后所有服务器的玩家都会在游戏内看到投票公告：
...
玩家输入 `/vote 1` 即可投票，同一玩家可以改票（后投的覆盖先投的）。
投票到期（默认 60 分钟，最长 7 天）后，第一个轮询的服务器会广播最终结果，
包括每个选项的票数、百分比与获胜选项：
...
发起投票的接口见 [`API.md`](API.md#post-apimc-bridgeactivity)。结果只广播一次，
之后可以继续用 `GET /api/mc-bridge/activity` 查询存档。
