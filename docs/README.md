...
玩家在游戏内输入 `/report <玩家名> <原因>` 可举报其他玩家。举报会自动发送到论坛，
论坛管理员可以在后台查看并处理。
...
举报需要 `mcbridge.report` 权限（默认所有玩家都有）。举报成功后玩家会收到确认消息：

```
已举报玩家 Steve，论坛管理员会尽快处理。
```

论坛侧的举报记录存储在 `mc_reports` 表中，状态为 `pending`。管理员可以手动将状态改为
`reviewed`（已处理）或 `dismissed`（已驳回）。

### 3.4 论坛投票联动（fof/polls）

游戏内投票**不另起一套系统**，而是直接对接论坛已有的 **fof/polls** 扩展：投票在论坛
界面创建和管理，游戏内负责广播和收票，两边看的是同一份数据。

**前置条件**

1. 论坛已安装 `fof/polls`（没装也不会报错，只是游戏内会提示投票不可用）；
2. 在论坛后台开启**全局投票**（`fof-polls.enableGlobalPolls`）—— 只有全局投票
   （在 `/polls` 页面创建的独立投票）会进入游戏，讨论内投票属于具体帖子，不广播；
3. 玩家要投票必须先 `/bind` 绑定论坛账号：票要算在具体用户名下，没有绑定就没有
   可以记账的对象。

**流程**

1. 管理员在论坛 `/polls` 页面创建全局投票，填好选项与结束时间；
2. 插件轮询到新投票后，在**所有服务器**的游戏内广播：
...
3. 玩家输入 `/vote 1` 投票（多选投票可用 `/vote 1 3` 或 `/vote 1,3`）。
   插件把票记到**该玩家绑定的论坛账号**名下，所以论坛页面上能看到他确实投了票；
4. 投票结束后，最近 24 小时内结束的投票会带最终票数在游戏内广播一次：
...
单选/多选、最大可选数量、能否改票、是否已结束这些规则**全部由 fof/polls 判定**，
插件不重新实现一遍——所以游戏内投票的行为与论坛网页完全一致。

接口细节见 [`API.md`](API.md#get-apimc-bridgepolls)。
